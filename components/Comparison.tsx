
import React from 'react';

const Comparison: React.FC = () => {
  const categories = [
    {
      feature: "The Workload",
      diy: "You do it all",
      bureau: "You upload files",
      maple: "We do it for you"
    },
    {
      feature: "CRA/Remittances",
      diy: "High risk & stress",
      bureau: "Automated but rigid",
      maple: "Guaranteed & Handled"
    },
    {
      feature: "Team Support",
      diy: "You answer questions",
      bureau: "A computer answers",
      maple: "Your expert answers"
    },
    {
      feature: "Year-End Stress",
      diy: "Manual T4 audit",
      bureau: "Batch processing",
      maple: "White-glove care"
    },
    {
      feature: "Benefits Sync",
      diy: "Total manual slog",
      bureau: "Partial sync",
      maple: "Full reconciliation"
    },
    {
      feature: "Legal Liability",
      diy: "100% on you",
      bureau: "Limited warranty",
      maple: "Full Coverage Guarantee"
    }
  ];

  return (
    <section id="comparison" className="py-16 bg-slate-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <h2 className="text-4xl lg:text-5xl font-black text-slate-900 mb-6 uppercase tracking-tighter">WHY PEOPLE <span className="text-red-600">CHOOSE US</span></h2>
          <p className="text-xl text-slate-600 max-w-2xl mx-auto font-medium">High-growth Canadian companies deserve more than just a software login.</p>
        </div>

        <div className="overflow-x-auto shadow-2xl">
          <table className="w-full text-left border-collapse bg-white rounded-none overflow-hidden border border-slate-200">
            <thead>
              <tr className="bg-slate-900 text-white">
                <th className="px-8 py-6 font-black uppercase tracking-[0.2em] text-[10px] border-r border-slate-800">What it feels like</th>
                <th className="px-8 py-6 font-black uppercase tracking-[0.2em] text-[10px] border-r border-slate-800">In-House / DIY</th>
                <th className="px-8 py-6 font-black uppercase tracking-[0.2em] text-[10px] border-r border-slate-800">Traditional Bureau</th>
                <th className="px-8 py-6 font-black uppercase tracking-[0.2em] text-[10px] bg-red-600">Maple Managed</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {categories.map((cat, i) => (
                <tr key={i} className="hover:bg-slate-50 transition-colors group">
                  <td className="px-8 py-6 font-bold text-slate-900 text-sm border-r border-slate-100 uppercase tracking-tight">{cat.feature}</td>
                  <td className="px-8 py-6 text-slate-500 text-sm italic border-r border-slate-100">{cat.diy}</td>
                  <td className="px-8 py-6 text-slate-500 text-sm italic border-r border-slate-100">{cat.bureau}</td>
                  <td className="px-8 py-6 text-slate-900 font-black text-sm bg-red-50/30 group-hover:bg-red-50 transition-colors">{cat.maple}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
        
        <div className="mt-12 text-center">
          <p className="text-slate-400 text-sm italic">
            "Software is just a tool. We are your partner."
          </p>
        </div>
      </div>
    </section>
  );
};

export default Comparison;
